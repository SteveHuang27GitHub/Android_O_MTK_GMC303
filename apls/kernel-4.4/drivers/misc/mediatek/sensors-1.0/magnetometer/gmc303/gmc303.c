...
/* gmc303.c - gmc303 compass driver
 *
 *
...
#define DEBUG 0
#define GMC303_DEV_NAME	"gmc303"
#define DRIVER_VERSION	 "1.0.1"
#define GMC303_DEBUG	1
#define GMC303_RETRY_COUNT	10
#define GMC303_DEFAULT_DELAY	100


#if GMC303_DEBUG
#define MAGN_TAG		 "[GMC303] "
...
static short akmd_delay = GMC303_DEFAULT_DELAY;
static int factory_mode;
static int gmc303_init_flag;
static struct i2c_client *this_client;
static int8_t akm_device;

static uint8_t akm_fuse[3] = {0};
/*----------------------------------------------------------------------------*/
static const struct i2c_device_id gmc303_i2c_id[] = { {GMC303_DEV_NAME, 0}, {} };

/* Maintain  cust info here */
struct mag_hw gmc303_mag_cust;
static struct mag_hw *hw = &gmc303_mag_cust;

/* For  driver get cust info */
struct mag_hw *gmc303_get_cust_mag(void)
{
	return &gmc303_mag_cust;
}

/*----------------------------------------------------------------------------*/
static int gmc303_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id);
static int gmc303_i2c_remove(struct i2c_client *client);
static int gmc303_i2c_detect(struct i2c_client *client, struct i2c_board_info *info);
static int gmc303_suspend(struct device *dev);
static int gmc303_resume(struct device *dev);
static int gmc303_local_init(void);
static int gmc303_remove(void);
static int gmc303_flush(void);

static struct mag_init_info gmc303_init_info = {
	.name = "gmc303",
	.init = gmc303_local_init,
	.uninit = gmc303_remove,
};
...
static const struct dev_pm_ops gmc303_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(gmc303_suspend, gmc303_resume)
};
#endif

static struct i2c_driver gmc303_i2c_driver = {
	.driver = {
		   .name = GMC303_DEV_NAME,
#ifdef CONFIG_PM_SLEEP
		.pm    = &gmc303_pm_ops,
#endif
#ifdef CONFIG_OF
	...
	.probe = gmc303_i2c_probe,
	.remove = gmc303_i2c_remove,
	.detect = gmc303_i2c_detect,
	.id_table = gmc303_i2c_id,
};
...
static DEFINE_MUTEX(gmc303_i2c_mutex);
...
#endif
static void gmc303_power(struct mag_hw *hw, unsigned int on)
{
}
...
static long GMI2C_RxData(char *rxData, int length)
{
	...
	int i = 0;
	struct i2c_client *client = this_client;
	struct gmc303_i2c_data *data = i2c_get_clientdata(client);
	char addr = rxData[0];
#endif
	...
	mutex_lock(&gmc303_i2c_mutex);
	for (loop_i = 0; loop_i < GMC303_RETRY_COUNT; loop_i++) {
	...
	if (loop_i >= GMC303_RETRY_COUNT) {
		mutex_unlock(&gmc303_i2c_mutex);
		MAGN_PR_ERR("%s retry over %d\n", __func__, GMC303_RETRY_COUNT);
		return -EIO;
	}
	mutex_unlock(&gmc303_i2c_mutex);
#if DEBUG
...
static int GMECS_CheckDevice(void)
{
	...
	ret = GMECS_ReadFuse();
	if (ret < 0) {
		MAGN_PR_ERR("GMC303 gmc303_probe: read fuse fail\n");
		return -ENXIO;
	}
...
static long GMECS_GetData(char *rbuf, int size)
{
	...
	if (ret < 0) {
		MAGN_PR_ERR("GMC303 gmc303_work_func: I2C failed\n");
		return -1;
	}
	rbuf[0] = temp;
...
/*----------------------------------------------------------------------------*/
static int gmc303_ReadChipInfo(char *buf, int bufsize)
{
	if ((!buf) || (bufsize <= GMC303_BUFSIZE - 1))
		return -1;
 ...
 */
int GMC303_TEST_DATA(const char testno[], const char testname[], const int testdata,
...
int FST_GMC303(void)
{
	...
	/* TEST */
	GMC303_TEST_DATA(TLIMIT_NO_RST_WIA1_60X, TLIMIT_TN_RST_WIA1_60X, (int)i2cData[0],
		  TLIMIT_LO_RST_WIA1_60X, TLIMIT_HI_RST_WIA1_60X, &pf_total);
	GMC303_TEST_DATA(TLIMIT_NO_RST_WIA2_60X, TLIMIT_TN_RST_WIA2_60X, (int)i2cData[1],
	...
	/* TEST */
	GMC303_TEST_DATA(TLIMIT_NO_ASAX_60X, TLIMIT_TN_ASAX_60X, asax, TLIMIT_LO_ASAX_60X,
		  TLIMIT_HI_ASAX_60X, &pf_total);
	GMC303_TEST_DATA(TLIMIT_NO_ASAY_60X, TLIMIT_TN_ASAY_60X, asay, TLIMIT_LO_ASAY_60X,
		  TLIMIT_HI_ASAY_60X, &pf_total);
	GMC303_TEST_DATA(TLIMIT_NO_ASAZ_60X, TLIMIT_TN_ASAZ_60X, asaz, TLIMIT_LO_ASAZ_60X,
	...
	/* TEST */
	i2cData[0] &= 0x7F;
	GMC303_TEST_DATA(TLIMIT_NO_SNG_ST1_60X, TLIMIT_TN_SNG_ST1_60X, (int)i2cData[0],
		  TLIMIT_LO_SNG_ST1_60X, TLIMIT_HI_SNG_ST1_60X, &pf_total);

	/* TEST */
	GMC303_TEST_DATA(TLIMIT_NO_SNG_HX_60X, TLIMIT_TN_SNG_HX_60X, hdata[0], TLIMIT_LO_SNG_HX_60X,
		  TLIMIT_HI_SNG_HX_60X, &pf_total);
	GMC303_TEST_DATA(TLIMIT_NO_SNG_HY_60X, TLIMIT_TN_SNG_HY_60X, hdata[1], TLIMIT_LO_SNG_HY_60X,
		  TLIMIT_HI_SNG_HY_60X, &pf_total);
	GMC303_TEST_DATA(TLIMIT_NO_SNG_HZ_60X, TLIMIT_TN_SNG_HZ_60X, hdata[2], TLIMIT_LO_SNG_HZ_60X,
		  TLIMIT_HI_SNG_HZ_60X, &pf_total);
	GMC303_TEST_DATA(TLIMIT_NO_SNG_ST2_60X, TLIMIT_TN_SNG_ST2_60X, (int)i2cData[8],
	...
	/* TEST */
	i2cData[0] &= 0x7F;
	GMC303_TEST_DATA(TLIMIT_NO_SLF_ST1_60X, TLIMIT_TN_SLF_ST1_60X, (int)i2cData[0],
 ...
 *@retval 0 The test is aborted by kind of system error.
 */
int gmc303_FctShipmntTestProcess_Body(void)
{
	int pf_total = 1;
	...
	/* Reset Test Result */
	/* *********************************************** */
	GMC303_TEST_DATA(NULL, "START", 0, 0, 0, &pf_total);
	...
	/* Judge Test Result */
	/* *********************************************** */
	GMC303_TEST_DATA(NULL, "END", 0, 0, 0, &pf_total);
...
static ssize_t show_shipment_test(struct device_driver *ddri, char *buf)
{
	...
	res = gmc303_FctShipmntTestProcess_Body();
	if (res == 1) {
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	gmc303_ReadChipInfo(strbuf, GMC303_BUFSIZE);
	return sprintf(buf, "%s\n", strbuf);
}
...
{
	ssize_t res;
	struct gmc303_i2c_data *obj = i2c_get_clientdata(this_client);

	if (obj == NULL) {
		MAGN_PR_ERR("gmc303_i2c_data is null!!\n");
		return 0;
	}
...
static ssize_t store_chip_orientation(struct device_driver *ddri, const char *buf, size_t tCount)
{
	int _nDirection = 0;
	int ret = 0;
	struct gmc303_i2c_data *_pt_i2c_obj = i2c_get_clientdata(this_client);
...
static ssize_t show_regiter_map(struct device_driver *ddri, char *buf)
{
	...
	u8  _baRegMap[] = 
#ifdef GMC303
{0x00,0x01,0x02,0x03,0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x31,0x32};
#endif
#ifdef GMC306
{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x10, 0x11, 0x12, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55};
#endif
   /* u8  _baRegValue[20]; */
	ssize_t	_tLength	 = 0;
	char tmp[2] = {0};
...
/*----------------------------------------------------------------------------*/
static struct driver_attribute *gmc303_attr_list[] = {
	&driver_attr_daemon,
...
/*----------------------------------------------------------------------------*/
static int gmc303_create_attr(struct device_driver *driver)
{
	int idx, err = 0;
	int num = (int)(ARRAY_SIZE(gmc303_attr_list));

	if (driver == NULL)
		return -EINVAL;

	for (idx = 0; idx < num; idx++) {
		err = driver_create_file(driver, gmc303_attr_list[idx]);
		if (err) {
			MAGN_PR_ERR("driver_create_file (%s) = %d\n",
				 gmc303_attr_list[idx]->attr.name, err);
			break;
		}
	}
	return err;
...
/*----------------------------------------------------------------------------*/
static int gmc303_delete_attr(struct device_driver *driver)
{
	int idx, err = 0;
	int num = (int)(ARRAY_SIZE(gmc303_attr_list));
	...
	for (idx = 0; idx < num; idx++)
		driver_remove_file(driver, gmc303_attr_list[idx]);
...
static int gmc303_suspend(struct device *dev)
{
	...
	struct gmc303_i2c_data *obj = i2c_get_clientdata(client);

	gmc303_power(obj->hw, 0);
...
static int gmc303_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	strlcpy(info->type, GMC303_DEV_NAME, sizeof(info->type));
	return 0;
}

static int gmc303_enable(int en)
{
	int value = 0;
	int err = 0;
	struct gmc303_i2c_data *f_obj = i2c_get_clientdata(this_client);
	...
	if (f_obj->flush) {
		if (value == 1) {
			MAGN_LOG("will call gmc303_flush in gmc303_enable\n");
			gmc303_flush();
		} else
			f_obj->flush = false;
	}
...
static int gmc303_set_delay(u64 ns)
{
	int value = 0;
...
static int gmc303_open_report_data(int open)
{
	return 0;
}

static int gmc303_coordinate_convert(int16_t *mag_data)
{
	struct i2c_client *client = this_client;
	struct gmc303_i2c_data *data = i2c_get_clientdata(client);
	int16_t temp_data[3];
...
static int gmc303_get_data(int *x, int *y, int *z, int *status)
{
	char i2cData[SENSOR_DATA_SIZE];
	int16_t mag[3];
	//static long int sx,sy,sz,count=0;
	GMECS_SetMode_SngMeasure();
	mdelay(10);
...
static int gmc303_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	int value = 0;
	...
	MAGN_LOG("gmc303 mag set delay = (%d) ok.\n", value);
	return 0;
}
...
static int gmc303_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	int err;

	err = gmc303_enable(enabledisable == true ? 1 : 0);
	if (err) {
		MAGN_PR_ERR("%s enable sensor failed!\n", __func__);
		return -1;
	}
	err = gmc303_batch(0, sample_periods_ms * 1000000, 0);
	if (err) {
...
static int gmc303_factory_get_data(int32_t data[3], int *status)
{
	/* get raw data */
	return  gmc303_get_data(&data[0], &data[1], &data[2], status);
}
static int gmc303_factory_get_raw_data(int32_t data[3])
{
	MAGN_LOG("do not support gmc303_factory_get_raw_data!\n");
	return 0;
}
static int gmc303_factory_enable_calibration(void)
{
	return 0;
}
static int gmc303_factory_clear_cali(void)
{
	return 0;
}
static int gmc303_factory_set_cali(int32_t data[3])
{
	return 0;
}
static int gmc303_factory_get_cali(int32_t data[3])
{
	return 0;
}
static int gmc303_factory_do_self_test(void)
{
	return 0;
}

static struct mag_factory_fops gmc303_factory_fops = {
	.enable_sensor = gmc303_factory_enable_sensor,
	.get_data = gmc303_factory_get_data,
	.get_raw_data = gmc303_factory_get_raw_data,
	.enable_calibration = gmc303_factory_enable_calibration,
	.clear_cali = gmc303_factory_clear_cali,
	.set_cali = gmc303_factory_set_cali,
	.get_cali = gmc303_factory_get_cali,
	.do_self_test = gmc303_factory_do_self_test,
};

static struct mag_factory_public gmc303_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &gmc303_factory_fops,
};
...
static int gmc303_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct gmc303_i2c_data *data;
	struct mag_control_path ctl = { 0 };
	struct mag_data_path mag_data = { 0 };

	MAGN_LOG("gmc303_i2c_probe\n");
	...
	data = kzalloc(sizeof(struct gmc303_i2c_data), GFP_KERNEL);
	if (!data) {
		err = -ENOMEM;
		goto exit;
	}
	...
	/*now use gmc303_coordinate_convert api, so the rang is 0-7 */
	data->hw->direction=0;
	...
	if (err < 0) {
		MAGN_PR_ERR("GMC303 gmc303_probe: check device connect error\n");
		goto exit_init_failed;
	}

	err = mag_factory_device_register(&gmc303_factory_device);
	if (err) {
	...
	err = gmc303_create_attr(&(gmc303_init_info.platform_diver_addr->driver));
	if (err) {
	...
	ctl.enable = gmc303_enable;
	ctl.set_delay = gmc303_set_delay;
	ctl.open_report_data = gmc303_open_report_data;
	ctl.batch = gmc303_batch;
	ctl.flush = gmc303_flush;
	...
	mag_data.get_data = gmc303_get_data;
...
static int gmc303_i2c_remove(struct i2c_client *client)
{
	...
	err = gmc303_delete_attr(&(gmc303_init_info.platform_diver_addr->driver));
	if (err)
		MAGN_PR_ERR("gmc303_delete_attr fail: %d\n", err);

	this_client = NULL;
	i2c_unregister_device(client);
	mag_factory_device_deregister(&gmc303_factory_device);
...
static int gmc303_remove(void)
{
	...
	i2c_del_driver(&gmc303_i2c_driver);
	return 0;
}
...
{
	gmc303_power(hw, 1);
	if (i2c_add_driver(&gmc303_i2c_driver)) {
		MAGN_PR_ERR("i2c_add_driver error\n");
		return -1;
	}
	if (-1 == gmc303_init_flag)
		return -1;
	return 0;
}

/*----------------------------------------------------------------------------*/
static int __init gmc303_init(void)
{
	mag_driver_add(&gmc303_init_info);
	return 0;
}

/*----------------------------------------------------------------------------*/
static void __exit gmc303_exit(void)
{
...
/*----------------------------------------------------------------------------*/
module_init(gmc303_init);
module_exit(gmc303_exit);

MODULE_AUTHOR("MTK");
MODULE_DESCRIPTION("GMC303 compass driver");
MODULE_LICENSE("GPL");
